...
type BorrowAuthorizationReportPDFParams = {
  data: BorrowAuthorizationReportDataSpec;
};

// Create Document Component
const BorrowAuthorizationReportPDF = ({ data }: BorrowAuthorizationReportPDFParams) => (
  <Document>
    <Page style={styles.page}>
      <View style={styles.container}>
        <View style={styles.section} fixed>
          <Image src={`${process.env.PUBLIC_URL}/logo.png`} style={{ width: '100px', height: '100px' }} />
          <Text style={styles.title}>AUTORIZACIÓN DE DESCUENTO POR CRÉDITO</Text>
        ...
        <View style={styles.section}>
          ...
          <Text style={styles.cell}>{`${ToMoney(data.requestedAmount)} (${data.requestedAmountInWords})`}</Text>
        </View>
        ...
        <Text>
          AUTORIZO EL DESCUENTO QUINCENAL VÍA NÓMINA POR LA CANTIDAD DE:
        </Text>

        <View style={styles.padding}></View>
        
        <Text>
          {`${ToMoney(data.requestedAmount)} (${data.paymentInWords})`}
        </Text>

        <View style={styles.padding}></View>

        <Text>
          POR CONCEPTO DE PRÉSTAMO, A PARTIR DE: {data.startAt}
        </Text>
        ...
        <View style={{ textAlign: 'center'}}>
          <Text>FIRMA Y NOMBRE COMPLETO: _______________________</Text>
        </View>
      </View>
    </Page>
  </Document>
...
export default BorrowAuthorizationReportPDF;